...
function CreateBoardModal({ projectId, onClose, onCreate }: { projectId: string; onClose: () => void; onCreate: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "min(420px, 94vw)", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{ padding: "24px 24px 0 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#0f172a" }}>Create Board</h2>
            <p style={{ margin: "4px 0 0 0", fontSize: 13, color: "#64748b" }}>Add a new Kanban board to this project</p>
          </div>
        ...
        <div style={{ padding: "20px 24px 24px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#1e293b", marginBottom: 6 }}>Board Name</label>
            <input
              value={name}
...
export default function BoardsTab({ projectId, boards, isAdmin, refreshProject }: BoardsTabProps) {
  ...
  const handleDelete = async (e: React.MouseEvent, boardId: string, boardName: string) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Failed to delete board. Make sure all tasks are removed first.");
    } finally {
      setDeletingId(null);
    }
  };
      ...
      {boards.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "#94a3b8" }}>
          ...
          {isAdmin && (
            <button
              onClick={() => setShowCreateModal(true)}
              style={{ background: "#6366f1", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", cursor: "pointer", fontWeight: 600 }}
            >
              + Create First Board
            </button>
          )}
              ...
              <div style={{ padding: "16px 18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <h4 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#1e293b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {b.name}
                    </h4>
                  ...
                  {isAdmin && (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 6, border: "1px solid #fee2e2",
                ...
                <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 999, background: "#eef2ff", color: "#6366f1", fontWeight: 600 }}>
                    Kanban
                  </span>
                </div>
              </div>
