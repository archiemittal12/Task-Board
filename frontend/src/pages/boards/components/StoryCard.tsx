...
export default function StoryCard({ story, taskCount, doneCount, allTasks, onEdit, onDelete }: StoryCardProps) {
  ...
  const progress = taskCount === 0 ? 0 : Math.round((doneCount / taskCount) * 100);
   ...
   const status = children.length > 0 ? deriveStoryStatus(children) : ((story.status as string) || "TODO");
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              ...
              <span style={{
                ...
                background: statusStyle.bg, color: statusStyle.text, fontWeight: 700,
              }}>
                {status.replace("_", " ")}
              </span>
              {/* Priority badge */}
              {story.priority && (
                <span style={{
                  ...
                  color: priorityColors[story.priority], fontWeight: 700,
                }}>
                  {story.priority}
                </span>
              )}
            </div>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#0f172a" }}>{story.title}</h3>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, marginLeft: 8, flexShrink: 0 }}>
            <button
              ...
              style={{ background: "linear-gradient(135deg, #6366f1, #4f46e5)", border: "none", borderRadius: 8, padding: "6px 8px", color: "#fff", cursor: "pointer", fontSize: 12 }}
            >
              ✏️
            </button>
            <button
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 12, color: "#64748b" }}>{doneCount}/{taskCount} done</span>
            <span style={{ fontSize: 12, color: "#6366f1", fontWeight: 600 }}>{progress}%</span>
          </div>
          <div style={{ height: 6, background: "#e2e8f0", borderRadius: 10 }}>
            <div style={{ width: `${progress}%`, height: "100%", background: progress === 100 ? "#22c55e" : "#6366f1", borderRadius: 10, transition: "width 0.3s" }} />
          </div>
        </div>
      ...
      {expanded && children.length > 0 && (
        <div style={{ borderTop: "1px solid #e2e8f0", padding: "10px 16px", background: "#f8fafc", display: "flex", flexDirection: "column", gap: 6 }}>
          {children.map(task => {
              ...
              <div key={task.id} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: tc.bg, color: tc.text, fontWeight: 700 }}>
                      {task.type}
                    </span>
                    <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: sc.bg, color: sc.text, fontWeight: 600 }}>
