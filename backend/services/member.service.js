...
const addMember = async (req, res) => {
  try {
    ...
    if (!['ADMIN', 'MEMBER', 'VIEWER'].includes(role)) {
      return res
        .status(400)
        .json({ success: false, message: 'Invalid role. Must be ADMIN, MEMBER, or VIEWER' });
    }
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ success: false, message: 'Only project admin can manage members' });
    }
    ...
    if (existing) {
      return res
        .status(400)
        .json({ success: false, message: 'User is already a member of this project' });
    }
  ...
  try {
    ...
    if (existing.role === 'ADMIN' && role !== 'ADMIN') {
      ...
      if (adminCount === 1) {
        return res
          .status(400)
          .json({ success: false, message: 'Cannot demote the last admin of a project' });
      }
    }
...
const removeMember = async (req, res) => {
  try {
    ...
    if (userId === requesterId) {
      return res
        .status(400)
        .json({ success: false, message: 'You cannot remove yourself from the project' });
    }
    ...
    if (existing.role === 'ADMIN') {
      ...
      if (adminCount === 1) {
        return res
          .status(400)
          .json({ success: false, message: 'Cannot remove the last admin of a project' });
      }
    }
