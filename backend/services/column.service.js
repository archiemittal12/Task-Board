...
const ensureUniqueColumnName = async (boardId, name, excludeId = null) => {
  ...
  const isDuplicate = columns.some(
    (col) => col.id !== excludeId && normalizeName(col.name) === normalizeName(name)
  );
  if (isDuplicate) {
...
//
const calculatePosition = async ({ boardId, beforeColumnId, afterColumnId }) => {
  const MIN_GAP = 10;
...
const updateColumn = async (req, res) => {
  try {
    ...
    if (status && status !== column.status) {
      ...
      const storyIds = [...new Set(affectedTasks.filter((t) => t.parentId).map((t) => t.parentId))];
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.columnTransition.deleteMany({
        where: {
          OR: [{ fromColumnId: columnId }, { toColumnId: columnId }],
        },
      });
