...
const calculatePosition = async ({ columnId, beforeTaskId, afterTaskId }) => {
  const MIN_GAP = 10;
...
const createWorkItem = async (req, res) => {
  try {
    ...
    const { title, description, priority, dueDate, assigneeId, parentId } = req.body;
...
const moveTask = async (req, res) => {
  try {
    ...
    if (targetColumnId !== task.columnId) {
      const notifyIds = [task.assigneeId, task.reporterId].filter((id) => id && id !== userId);
